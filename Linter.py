...
class Config:
    ...
    def get_state(self, index: int) -> str: return self.settings[index]
...
class Linter:
    ...
    def __check_spacing(self) -> list:
        errors = []
        errors.append(self.__find_errors(fr'\s{{{self.config.get_state(0)},}}'))
        if self.config.get_state(1) == "true":
            errors.append(self.__find_errors(
                r"[\w\d]+[!=\+\-\*\/%]=|[!=\+\-\*\/%]=[\w\d]+|[\w\d]+[\/=\*<>&|\+\-]|[\/=\*<>&|\+\-][\w\d]+",
                what_check="around_operators"))
        if self.config.get_state(2) == "true":
            errors.append(self.__find_errors(r",\w+",
                                             what_check="after_commas"))
        if self.config.get_state(3) == "true":
            errors.append(self.__find_errors(r"if\(|while\(|for\(",
                                             what_check="around_keywords"))
        ...
        for i, new_line in enumerate(self.formated_java_doc):
            if new_line == "":
                ...
                if count >= int(self.config.get_state(4)):
                    errors.append(f"Слишком мнгого пустых строк подряд в строке {i + 1}")
            else:
                count = 0
...
#
        if self.config.get_state(5) == "camelCase":
            errors.append(self.__find_errors(r"[A-Z]?[a-z]+_[A-Za-z]+|\d+[a-zA-Z]+|_[a-z]+|\b[A-Z]\b",
                                             what_check="variables_naming"))
        if self.config.get_state(6) == "UPPER_CASE":
            #final\s+\w+\s+[a-zA-Z][a-zA-Z]+
            errors.append(self.__find_errors(r"final\s\w+\s[^A-Z]+|final\s\w+\s[^a-z][a-z]",
                                             what_check="const_naming"))
        if self.config.get_state(7) == "camelCase":
            errors.append(self.__find_errors(r"[a-z]+_[a-z]+\(|\d+[a-zA-Z]+\(|_[a-z]+\(|[A-Z][a-z]+[A-Z][a-z]+\(",
                                             what_check="methods_naming"))
        if self.config.get_state(8) == "PascalCase":
            errors.append(self.__find_errors(r"class\s[a-z]",
                                             what_check="classes_naming"))
            ...
            if re.search(reg, new_line):
                match what_check:
                    case "around_operators":
                        errors.append(f"Не установлены пробелы вокруг оператора в строке {i + 1}: '{new_line}'")
                    case "after_commas":
                        errors.append(f"Не установлены пробелы после запятой {i + 1}: '{new_line}'")
                    case "around_braces":
                        errors.append(f"Лишний пробел после вызова метода {i + 1}: '{new_line}'")
                    case "around_keywords":
                        errors.append(f"Не установлены пробелы после ключеввых слов {i + 1}: '{new_line}'")
                    case "const_naming":
                        errors.append(f"Неправильное название константы в строке {i + 1}: '{new_line}'")
                    case "classes_naming":
                        errors.append(f"Неправильное название класса в строке {i + 1}: '{new_line}'")
                    case "variables_naming":
                        errors.append(f"Неправильное название переменной в строке {i + 1}: '{new_line}'")
                    case "methods_naming":
                        errors.append(f"Неправильное название метода в строке {i + 1}: '{new_line}'")
                    case _:
                        errors.append(f"Лишние пробелы в строке {i + 1}: '{new_line}'")
        return errors
